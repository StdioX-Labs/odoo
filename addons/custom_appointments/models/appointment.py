...
class Appointment(models.Model):
    ...
    def _generate_ics_attachment(self, for_staff=False):
        ...
        # Contact details must not cross between the two audiences: the customer's
        # copy is organised by the branch, and the staff copy carries no customer
        # address.
        if for_staff:
            organiser = self.staff_member_id.email
        else:
            organiser = self.branch_id.email or self.env.company.email
            if self.customer_email:
                event.add('attendee', f'MAILTO:{self.customer_email}')

        if organiser:
            event.add('organizer', f'MAILTO:{organiser}')
    ...
    def _send_staff_notification(self):
        ...
        for appointment in self:
            ...
            if appointment.staff_member_id.email:
                ...
                try:
                    ics_attachment = appointment._generate_ics_attachment(for_staff=True)
